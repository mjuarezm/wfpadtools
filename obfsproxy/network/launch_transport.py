...
def launch_transport_listener(transport, bindaddr, role, remote_addrport, pt_config, ext_or_cookie_file=None):
    ...
    if role == 'socks':
        ...
        factory = extended_orport.ExtORPortServerFactory(remote_addrport, ext_or_cookie_file, transport, transport_class, pt_config, transport)
    else:
        assert(remote_addrport)
        factory = network.StaticDestinationServerFactory(remote_addrport, role, transport_class, pt_config, transport)

    addrport = reactor.listenTCP(listen_port, factory, interface=listen_host)
    transport.server = addrport
